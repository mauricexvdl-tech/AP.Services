...
describe("AporiaRegistry", function () {
    ...
    describe("Registration", function () {
        it("should register a bot with initial deposit", async function () {
            ...
            const tx = await registry.connect(user1).registerBot(IMAGE_URI, ENCRYPTED_ENV, TIER_NANO, { value: deposit });
            const receipt = await tx.wait();

            // Extrahiere botId aus Event
            const event = receipt?.logs.find(
                (log) => registry.interface.parseLog({ topics: log.topics as string[], data: log.data })?.name === "BotRegistered"
            );
        ...
        it("should activate monitoring when deposit >= 2x restart cost", async function () {
            ...
            const parsedLog = receipt?.logs
                .map((log) => registry.interface.parseLog({ topics: log.topics as string[], data: log.data }))
    ...
    describe("Restart & Cooldown", function () {
        ...
        it("should trigger restart and emit event", async function () {
            await expect(registry.triggerRestart(botId))
                .to.emit(registry, "RestartTriggered");
        });
        ...
        it("should enforce 6-hour cooldown", async function () {
            ...
            await expect(registry.triggerRestart(botId))
                .to.be.revertedWithCustomError(registry, "CooldownActive");
        });
        ...
        it("should only allow owner (watchdog) to trigger restart", async function () {
            await expect(
                registry.connect(user1).triggerRestart(botId)
    ...
    describe("Kill Switch", function () {
        ...
        it("should only allow owner to ban images", async function () {
            await expect(
                registry.connect(user1).banImage(IMAGE_URI)
            ).to.be.revertedWithCustomError(registry, "OwnableUnauthorizedAccount");
        });
    });
