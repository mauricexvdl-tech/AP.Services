...
async function showBotStatus(registry: ethers.Contract, botId: string): Promise<void> {
    ...
    try {
        ...
        console.log(`│  Balance:   ${chalk.green(ethers.formatEther(details.balance))} ETH             │`);
...
async function showAllBots(
    ...
    try {
        ...
        if (myBots.length === 0) {
            ...
            console.log(chalk.cyan(`  Run ${chalk.bold("aporia register")} to register your first bot.\n`));
            return;
        }

        console.log(chalk.white(`  Found ${chalk.bold(String(myBots.length))} bot(s) for ${wallet.address}\n`));
