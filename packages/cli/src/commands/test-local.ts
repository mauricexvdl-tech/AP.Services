...
export async function testLocalCommand(): Promise<void> {
  ...
  try {
    ...
    deploySpinner.succeed(
      `Bot deployed: ${chalk.green(containerName)} (${currentContainerId.substring(0, 12)})`,
    );
  ...
  console.log(
    chalk.gray(
      `   Interval: ${HEALTH_CHECK_INTERVAL_MS}ms  |  Timeout: ${HEALTH_CHECK_TIMEOUT_MS}ms  |  Max Failures: ${MAX_FAILURES}`,
    ),
  );
    ...
    for (let round = 1; round <= maxRounds; round++) {
      ...
      try {
        const res = await httpGet(
          `http://localhost:${BOT_PORT}${HEALTH_ENDPOINT}`,
      ...
      if (failureCount >= MAX_FAILURES && !botIsDown) {
        ...
        try {
          ...
          if (isHealthy) {
            resSpinner.succeed(
              `Resurrected: ${chalk.green(newName)} (${currentContainerId.substring(0, 12)})`,
            );
