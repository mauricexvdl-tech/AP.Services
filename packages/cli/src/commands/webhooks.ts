...
async function webhooksSetup(): Promise<void> {
    ...
    const answers = await inquirer.prompt([
        {
            ...
            validate: (input: string) => input.length > 0 || "Required. Generate at: https://dashboard.alchemy.com",
        },
        {
            type: "input",
            name: "webhookUrl",
    ...
    try {
        ...
        console.log(chalk.cyan("\n  Next: Run"), chalk.bold("aporia webhooks start"), chalk.cyan("to receive events.\n"));
...
async function webhooksStatus(): Promise<void> {
    ...
    console.log(chalk.gray(`    Webhook URL:    ${config.webhookUrl || chalk.yellow("Not configured")}`));
    console.log(chalk.gray(`    Signing Key:    ${config.webhookSigningKey ? config.webhookSigningKey.substring(0, 12) + "..." : chalk.yellow("Not configured")}`));
