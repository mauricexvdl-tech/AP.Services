...
export async function testTestnetCommand(): Promise<void> {
  ...
  console.log(
    chalk.gray(
      `  Deployer public key: ${publicKeyToBase64(deployerKeyPair.publicKey).substring(0, 20)}...`,
    ),
  );
  ...
  console.log(
    chalk.gray(
      `  Encrypted env: ${encryptedBytes.length} bytes (${Object.keys(dummyEnv).length} vars)`,
    ),
  );
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    if (
      ourBot &&
      ourBot.url &&
      !ourBot.url.includes("localhost") &&
      !ourBot.url.includes("unknown")
    ) {
      ...
      console.log(
        chalk.white(
          `  Last Restart: ${lastRestart > 0 ? new Date(lastRestart * 1000).toISOString() : "never"}`,
        ),
      );
