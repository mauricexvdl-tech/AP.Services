...
export const defaultHandlers: EventHandlerMap = {
  onBotRegistered: (event) => {
    console.log(
      `[Webhook] 🤖 BotRegistered: botId=${event.args.botId}, owner=${event.args.owner}, tier=${event.args.tier}`,
    );
  },
  onDeposited: (event) => {
    console.log(
      `[Webhook] 💰 Deposited: botId=${event.args.botId}, amount=${ethers.formatEther(event.args.amount)} ETH`,
    );
  },
  onWithdrawn: (event) => {
    console.log(
      `[Webhook] 💸 Withdrawn: botId=${event.args.botId}, amount=${ethers.formatEther(event.args.amount)} ETH`,
    );
  },
  onRestartTriggered: (event) => {
    console.log(
      `[Webhook] 🔄 RestartTriggered: botId=${event.args.botId}, timestamp=${event.args.timestamp}`,
    );
  },
  onUnknown: (event) => {
