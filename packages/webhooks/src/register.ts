...
export async function registerWebhook(options: RegisterWebhookOptions): Promise<WebhookInfo> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create webhook: HTTP ${response.status}\n${errorBody}`);
  }
 ...
 */
export async function deleteWebhook(authToken: string, webhookId: string): Promise<void> {
  const response = await fetch(`${NOTIFY_API_BASE}/delete-webhook?webhook_id=${webhookId}`, {
    method: "DELETE",
    headers: {
