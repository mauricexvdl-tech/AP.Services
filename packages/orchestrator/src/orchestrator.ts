...
export type ResurrectionPhase = "trigger" | "verify" | "fetch" | "decrypt" | "build" | "deploy" | "settle";
...
export class ResurrectionOrchestrator {
    ...
    async handleRestart(event: RestartEvent): Promise<ResurrectionResult> {
        ...
        if (this.activeResurrections.has(botId)) {
            return this.result(botId, "trigger", false, startTime,
        ...
        try {
            ...
            if (!botData) {
                return this.result(botId, "fetch", false, startTime,
                    "Failed to fetch bot data from chain");
            }
            ...
            try {
                ...
                console.log(`[Orchestrator]    ✅ Decrypted ${Object.keys(envVars).length} env vars (in RAM)`);
            } catch (error: any) {
                return this.result(botId, "decrypt", false, startTime,
            ...
            console.log(`[Orchestrator] 🚀 Phase 6/7: DEPLOY – Deploying via ${this.config.backend.name}...`);
            ...
            if (!deployResult.success) {
                return this.result(botId, "deploy", false, startTime,
            ...
            try {
                ...
                console.error(`[Orchestrator]       Container ${deployResult.deploymentId} is running but not settled on-chain`);
                return this.result(botId, "settle", false, startTime,
                    `Settlement failed (container running): ${error.message}`,
                    deployResult.deploymentId, deployResult.url);
            }
            ...
            console.log(`[Orchestrator]    Stats: ${this.stats.success}/${this.stats.total} successful\n`);
    ...
    private async verifyOnChain(botId: string): Promise<{ ok: boolean; error?: string }> {
        try {
            ...
            console.log(`[Orchestrator]    ✅ Balance: ${ethers.formatEther(balance)} ETH (cost: ${ethers.formatEther(cost)} ETH)`);
