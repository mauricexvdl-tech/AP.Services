...
export class OrchestratorService {
  ...
  constructor(config: ServiceConfig) {
    ...
    // Wire the monitor's restart callback into the orchestrator pipeline.
    // On successful resurrection, we update the bot's URL in the monitor
    // so subsequent health checks hit the new container, not the dead one.
    this.monitor = new HeartbeatMonitor(config.healthCheckConfig || {}, async (event) => {
      const result = await this.orchestrator.handleRestart(event);
      if (result.success && result.url) {
        // Re-register with updated URL so health checks target the new instance
  ...
  async start(): Promise<void> {
    ...
    // Populate the monitor with on-chain bot data before starting the loop
    await this.syncBots();

    // Begin HTTP health-check polling for all registered bots
    this.monitor.start();
    this.running = true;

    // Periodically reconcile with chain state (new registrations, deactivations)
