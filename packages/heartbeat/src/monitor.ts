...
export class HeartbeatMonitor {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `[Heartbeat] Config: interval=${this.config.intervalMs}ms, timeout=${this.config.timeoutMs}ms, maxFailures=${this.config.maxFailures}`,
    );
  ...
  async runHealthCheckRound(): Promise<HealthCheckResult[]> {
    ...
    const results = await Promise.allSettled(
      activeBots.map((bot) => limit(() => this.checkBot(bot))),
    );
